...
<<<<<<< HEAD
<<<<<<< acd44f89644ebca8121c0b26bfadc7ba48f392a4
					如果对象是复合值，而却大部分对象无法真正表示一个原始值，因此默认的value			法简单的返回对象本身，而不是返回原始值。
					数组、函数和正则表达式简单的继承了这个more方法，返回对象本身
=======
				      如果对象是复合值，而却大部分对象无法真正表示一个原始值，因此默认的valueof()方法简单的返回对象本身，而不是返回原始值。
				      数组、函数和正则表达式简单的继承了这个more方法，返回对象本身
...
<<<<<<< HEAD
<<<<<<< acd44f89644ebca8121c0b26bfadc7ba48f392a4
	setState通过一个队列机制实现state更新，当执行setState时，会将需要更新的state放入状态队列
=======
	setState通过一个队列机制实现state更新，当执行setState时，会将需要更新的state很后放入状态队列
>>>>>>> update:第一次提交笔记内容
=======
...
<<<<<<< HEAD
<<<<<<< acd44f89644ebca8121c0b26bfadc7ba48f392a4
	    因次，safair浏览器就会等待300ms，用来判断用户是否再次点击了屏幕
=======
	    因次，safair浏览器就会等待300ms，用来判断用户是否在次点击了屏幕
>>>>>>> update:第一次提交笔记内容
=======
